...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const results = requests.map((request: Request) => {
    const metadata = Object.entries(request.request_headers!).filter(([key, _]) => key.startsWith('x-metadata'));

    const { request_headers, ...updatedRequest } = request;

    const columns = [...Object.keys(updatedRequest), ...metadata.map(([key, _]) => key)];
  ...
  const columns = [...new Set(results.flatMap((result: any) => result.columns))];
