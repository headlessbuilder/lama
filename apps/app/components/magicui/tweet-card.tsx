...
import { enrichTweet, useTweet, type EnrichedTweet, type TweetProps, type TwitterComponents } from 'react-tweet';
  ...
  <div className="break-words leading-normal tracking-tighter">
    {tweet.entities.map((entity, idx) => {
      switch (entity.type) {
        ...
        case 'mention':
          return (
            <a key={idx} href={entity.href} target="_blank" rel="noopener noreferrer" className="text-gray-500">
              <span>{entity.text}</span>
            </a>
          );
        case 'text':
          return <span key={idx} className="text-sm" dangerouslySetInnerHTML={{ __html: entity.text }} />;
      }
    })}
  </div>
);
...
export const MyTweet = ({ tweet: t, components, className, ...props }: Props) => {
        ...
        {tweet.photos && (
          <div className="flex flex-row gap-2">
            {tweet.photos.map((photo) => (
              <img key={photo.url} src={photo.url} className="rounded-xl border shadow-sm" />
            ))}
          </div>
        )}
      </div>
