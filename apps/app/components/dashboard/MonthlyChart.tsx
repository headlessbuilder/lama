...
import { AreaChart, BarChart, Flex, Legend, Metric, Tab, TabGroup, TabList, Text, Title } from '@tremor/react';
...
const MonthlyChart = ({ startDate, endDate, categories }: MonthlyChartProps) => {
  const [selection, setSelection] = useLocalStorage<Select>(SELECTION_KEY, 'day');
  ...
  const totalUsage = dailyCosts.reduce((acc, cv) => {
    return acc + cv.line_items.reduce((acc, cv) => acc + (new Set(categories).has(cv.name as Model) ? cv.cost : 0), 0);
  }, 0);
  ...
  const data = dailyCosts.map((day) => {
    return {
      ...
      'Total Cost ($)': (day.line_items.reduce((acc, cv) => acc + cv.cost, 0) / 100).toFixed(2),
    };
  });
  ...
  const cumulativeData = dailyCosts.map((day, index) => {
    return {
        ...
        return {
          [category]: (
            dailyCosts
              .slice(0, index + 1)
              .reduce(
                (acc, cv) => acc + cv.line_items.reduce((acc, cv) => acc + (cv.name === category ? cv.cost : 0), 0),
                0,
              ) / 100
          ).toFixed(2),
        };
      ...
      'Total Cost ($)': (
        dailyCosts
          .slice(0, index + 1)
          .reduce((acc, cv) => acc + cv.line_items.reduce((acc, cv) => acc + cv.cost, 0), 0) / 100
      ).toFixed(2),
    };
  });
  // .filter((day) => parse(day.date, "MMM d", new Date()) >= startDate);
  ...
  return (
    <Card className="shadow-none">
      <CardHeader>
        ...
        <Text>{startDate && endDate && `from ${format(startDate, 'MMM d')} to ${format(endDate, 'MMM d')}`}</Text>
      </CardHeader>
      <CardContent>
        <Flex>
          <TabGroup
        ...
        {selection === 'cumulative' && cumulativeData.length > 0 && (
          <motion.div
            ...
            <AreaChart
              ...
              showLegend={false}
              showAnimation={false}
              maxValue={Math.max(...cumulativeData.map((d: any) => Math.max(...categories.map((c) => d[c]))))}
            />
            <Legend
              className="mt-4 mr-2.5"
              categories={categories}
