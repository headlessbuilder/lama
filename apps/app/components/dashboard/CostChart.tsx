...
const CostChart = ({ startDate, endDate, categories }: CostChartProps) => {
  ...
  const { snapshots, selectedSnapshots, data, loading } = useCostChartData(startDate, endDate, categories, false);

  if (!BillingData || !snapshots || !selectedSnapshots || !data) return <LoadingCard />;

  const dailyCosts = BillingData.daily_costs;

  const totalUsage = dailyCosts[dailyCosts.length - 1].line_items.reduce(
    (acc, cv) => acc + (new Set(categories).has(cv.name as Model) ? cv.cost : 0),
    0,
  );
