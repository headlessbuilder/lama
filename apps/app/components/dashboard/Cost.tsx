...
import { DonutChart, Flex, Metric, ProgressBar, Text, Title } from '@tremor/react';
...
const MonthlyCostChart = ({ startDate, endDate, categories }: MonthlyCostChartProps) => {
  const { data: subscription } = useSubscriptionData();
  const { chartData, billing, isLoading } = useCostDonutChartData(startDate, endDate, categories);
    ...
    <motion.div
      ...
      <Card className="shadow-none">
        <CardHeader>
          ...
          <Metric>
            {/* @ts-ignore */}$ {chartData.reduce((acc, { cost }) => acc + cost, 0).toFixed(2)}
          </Metric>
        </CardHeader>
        <CardContent>
          <DonutChart
          ...
          <Flex className="mt-4">
            <Text className="truncate">{`${getPercentage().toFixed(2)}% of hard limit`}</Text>
