...
import { ArrowDownTrayIcon, ArrowUpTrayIcon, ChartBarIcon, InformationCircleIcon } from '@heroicons/react/24/solid';
import { BarList, Bold, Flex, Icon, Metric, Tab, TabGroup, TabList, Text, Title } from '@tremor/react';
...
const LoadingList = () => {
  ...
  const getRandomHeight = () => {
    return loadingBarHeights[Math.floor(Math.random() * loadingBarHeights.length)];
  };
  const [heights, setHeights] = useState<number[]>(new Array(6).fill(0).map((_) => getRandomHeight()));
...
const selectedCategories: SelectedCategory[] = ['total', 'context', 'generated'];
...
const Tokens = ({
  ...
  const [selectedCategory, setSelectedCategory] = useState<SelectedCategory>('total');

  const { contextTokenData, generatedTokenData, totalTokenData, loading } = useUsageDataCumulative(startDate, endDate);
    ...
    <motion.div
      ...
      <Card className="shadow-none">
        ...
        <CardContent>
          <TabGroup
            className="mt-6"
            onIndexChange={(value) => setSelectedCategory(selectedCategories[value as number])}
