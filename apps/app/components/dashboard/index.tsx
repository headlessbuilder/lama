...
import { CATEGORIES, CATEGORIES_KEY, LOCAL_STORAGE_KEY, LOCAL_STORAGE_ORG_ID } from '@/lib/constants';
...
export default function Dashboard() {
  ...
  const [categories, setCategories] = useLocalStorage<Category[]>(CATEGORIES_KEY, CATEGORIES);
    ...
    <m.div
      ...
      <Flex className="flex-col items-start space-y-4 2xl:flex-row 2xl:items-center">
        ...
        <div className="z-20 flex flex-col items-end w-full max-w-3xl space-x-0 space-y-4 md:flex-row md:space-y-0 md:space-x-4">
          <DateRangePicker value={value} onValueChange={setDates} selectPlaceholder="Select">
          ...
          {users && (
            <Select onValueChange={(value) => console.log('The selected value is', value)} placeholder="Select User">
              {users.members.data.map(({ user }, idx) => (
                <SelectItem
                  key={idx}
                  value={user.id}
                  icon={() => <img alt={user.name} src={user.picture} className="mr-3 rounded-full w-7 h-7" />}>
                  {user.name}
                </SelectItem>
              ))}
            </Select>
          ...
          <TabPanel>
            <Grid className="gap-4 mt-6">
              <Grid numItems={1} numItemsMd={2} numItemsLg={4} className="gap-4">
                <CostChart startDate={new Date()} endDate={new Date()} categories={categories!} />
              ...
              <Grid numItemsMd={1} className="gap-4">
                <MonthlyChart startDate={value.from!} endDate={value.to!} categories={categories!} />
              </Grid>

              <Grid numItemsMd={1} numItemsLg={3} className="gap-4">
                <Cost startDate={value.from!} endDate={value.to!} categories={categories!} />
              </Grid>
            </Grid>
          </TabPanel>
          <TabPanel>
            <DetailedAnalyticsTab startDate={value.from!} endDate={value.to!} categories={categories!} />
          </TabPanel>
        </TabPanels>
      </TabGroup>
    </m.div>
