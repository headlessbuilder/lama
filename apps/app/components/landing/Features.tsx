  ...
  return (
    <>
      <div className="relative before:absolute before:inset-0 before:bg-[radial-gradient(ellipse_50%_50%_at_center,rgba(var(--feature-color),0.1),transparent)]">
        <Container className={cn('max-w-[90%] text-center', imageSize === 'small' ? 'w-[78rem]' : 'w-[102.4rem]')}>
      ...
      <Container className="w-[78rem] max-w-[90%] text-center">
        <p className="mx-auto my-16 text-2xl leading-tight md:w-[80%] md:text-4xl">{text}</p>
      ...
      <div
        ...
        {features.map(({ title, text, Image, imageClassName }) => (
          <div
            ...
            {typeof Image === 'string' && <img className={cn('absolute max-w-none', imageClassName)} src={Image} />}

            {typeof Image === 'function' && (
              <div className={cn('absolute inset-0 flex items-center justify-center', imageClassName)}>
                <Image />
              </div>
            )}
          </div>
