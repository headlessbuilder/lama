...
export async function SiteHeader({ user }: SiteHeaderProps) {
  const { stargazers_count: stars } = await fetch('https://api.github.com/repos/dillionverma/llm.report', {
          ...
          <div className="w-full flex-1 md:w-auto md:flex-none">
            <Link
              ...
              <div className="hidden md:flex items-center gap-1 text-sm text-gray-500">
                ...
                <CountingNumbers value={stars} className="font-display font-medium text-white" />
              </div>
            </Link>
          </div>
