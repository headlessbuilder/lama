  ...
  const handleCheckout = (plan: Name) => {
    ...
    if (plan === 'Enterprise') {
      ...
      console.log(
        ...
        subscriptionPlans[process.env.NODE_ENV as 'development' | 'production' | 'test'],
      );
      const paymentLink =
        subscriptionPlans[process.env.NODE_ENV as 'development' | 'production' | 'test'][
    ...
    <Suspense>
      <section className="container h-full w-full flex flex-col gap-4">
        <div className="relative max-w-xl mx-auto sm:text-center">
          <h3 className="text-gray-800 text-3xl font-semibold sm:text-4xl">Simple pricing.</h3>
          <div className="mt-3 max-w-xl">
            <p>
              <strong>100k logs free every month.</strong> No credit card required.
            </p>
          </div>
        </div>
        ...
        <div className="grid mt-8 items-start gap-4 sm:grid-cols-3 sm:space-y-0 xl:grid-cols-3">
          {plans.map((item, idx) => (
            <div
              key={idx}
              className={cn(`relative mt-6 grid grid-rows-5 h-full rounded-xl border divide-y sm:mt-0`, {
                ...
                <div className="flex flex-col justify-between">
                  <div className={`text-4xl font-semibold text-gray-800`}>
                    ...
                    <span className="text-xl font-normal text-gray-600">
                      {item.price === 'Contact Us' ? '' : <sub className="text-xs">/ month</sub>}
                    </span>
                  </div>
                </div>
