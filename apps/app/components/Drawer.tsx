...
const Drawer = () => {
  ...
  const projectedLogs = (logCount / new Date().getDate()) * getDaysInMonth(new Date());

  const renderLinks = (links: any) =>
    links.map((navItem: any, index: number) => (
      <LinkItem key={index} {...navItem} isActive={activeTab === navItem.href.replace('/', '')} />
    ));
    ...
    <aside className="flex-col flex-shrink-0 w-64 h-screen transition-transform -translate-x-full lg:translate-x-0 border-r border-b justify-between hidden lg:flex px-4 pt-4">
      ...
      <Card className="p-2">
        ...
        <CardContent className="p-2 flex flex-col gap-2">
          ...
          <div className="text-xs">{numberFormat(logsLeft)} logs left this month</div>
