...
import { cn, currencyFormat, numberFormat, truncate, truncateEmail } from '@/lib/utils';
                      ...
                      <Dialog.Panel className="relative w-screen max-w-[50vw] pointer-events-auto">
                        ...
                        <div className="flex flex-col h-full py-6 overflow-y-scroll bg-white shadow-xl">
                          <div className="px-4 sm:px-6">
                            <Dialog.Title className="text-base font-semibold leading-6 text-gray-900">Log</Dialog.Title>
                          </div>
                          {/* <pre>
                          ...
                          <div className="flex-1 px-4 mt-6 sm:px-6">
                            <table className="min-w-full divide-y divide-gray-200">
                              <tbody className="bg-white divide-y divide-gray-200">
                                <TR label="created at" value={new Date(request.createdAt).toLocaleString()} />
                                ...
                                <TR label="cost" value={currencyFormat(request.cost, 'USD', 6)} />
                                <TR label="prompt tokens" value={numberFormat(request.prompt_tokens)} />
                                <TR label="completion tokens" value={numberFormat(request.completion_tokens)} />

                                <TR label="cached" value={request.cached ? 'true' : 'false'} />
                                <TR label="streamed" value={request.streamed ? 'true' : 'false'} />
                                <TR label="user id" value={truncateEmail(request.user_id)} />
                                <TR label="app id" value={truncateEmail(request.app_id)} />
                            ...
                            <TabGroup defaultIndex={0} onIndexChange={(value) => setTab(TabStates[value])}>
                            ...
                            <div className="mt-4">
                              {tab === 'pretty' && (
                                <div className="space-y-4">
                                  <h3 className="text-lg font-medium leading-6 text-gray-900">Prompt</h3>
                                  <div className="p-4 text-sm whitespace-pre-wrap border border-gray-200 rounded-lg bg-gray-50">
                                    {new URL(request.url).pathname === '/v1/completions' && (
                                      <RenderMarkdown>{request.request_body.prompt}</RenderMarkdown>
                                    )}
                                  ...
                                  <h3 className="text-lg font-medium leading-6 text-gray-900">Completion</h3>
                                  <div className="p-4 text-sm border border-gray-200 rounded-lg bg-gray-50">
                                    {new URL(request.url).pathname === '/v1/completions' && (
                                      <>{request.response_body.choices?.[0].text || request.completion}</>
                                    )}
                                    {new URL(request.url).pathname === '/v1/chat/completions' && !request.streamed && (
                                      <RenderMarkdown>
                                        {request.response_body.choices?.[0].message.content || request.completion}
                                      </RenderMarkdown>
                                    )}
                                    {new URL(request.url).pathname === '/v1/chat/completions' && request.streamed && (
                                      <RenderMarkdown>{request.completion}</RenderMarkdown>
                                    )}
                                  </div>
                                </div>
                              )}
                              ...
                              {tab === 'raw' && (
                                <div className="space-y-4">
                                  ...
                                  <pre className="p-4 overflow-auto text-sm whitespace-pre-wrap bg-gray-100 rounded-lg">
                                    <code>{JSON.stringify(request.request_body, null, 2)}</code>
                                  </pre>
                                  <h3 className="text-lg font-medium leading-6 text-gray-900">Response</h3>
                                  <pre className="p-4 overflow-auto text-sm whitespace-pre-wrap bg-gray-100 rounded-lg">
                                    <code>
                                      {!request.streamed && <>{JSON.stringify(request.response_body, null, 2)}</>}

                                      {request.streamed && (
                                        <RenderMarkdown>{request.streamed_response_body}</RenderMarkdown>
                                      )}
                                    </code>
                                  </pre>
                                </div>
  ...
  // },
  {
    ...
    cell: ({ row }) => {
      ...
      if (path === '/v1/completions') {
        ...
        prompt = (row.getValue('request_body') as any)?.messages.map((message: any) => `${message.content}`).join('\n');
      }
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem
              onClick={(e) => {
                ...
                navigator.clipboard.writeText(JSON.stringify(request.request_body));
              }}>
              Copy Request (JSON)
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={(e) => {
                e.stopPropagation();
                if (request.streamed) {
                  navigator.clipboard.writeText(JSON.stringify(request.streamed_response_body));
                } else {
                  navigator.clipboard.writeText(JSON.stringify(request.response_body));
                }
              }}>
              Copy Response (JSON)
            </DropdownMenuItem>
...
export function RequestTable({ userId }: { userId?: string }) {
  ...
  return (
    <>
      ...
      <RequestDialog request={selectedRequest} isOpen={isOpen} closeModal={closeModal} />
