  ...
  useEffect(() => {
    ...
    fetch(apiUrl)
      ...
      .then((data) => {
        ...
        const grouped: Record<string, { completion_tokens: number; prompt_tokens: number; cost: number }> =
          data.requests
            .filter(
              (request: Request) =>
        ...
        const groupedArray = Object.entries(grouped).map(([date, { completion_tokens, prompt_tokens, cost }]) => ({
          createdAt: date,
          completion_tokens,
          prompt_tokens,
          cost,
