...
export const manageSubscriptionStatusChange = async (
  ...
  console.log(`User [${user?.id}] is changing subscription [${subscriptionId}] wth customer [${customerId}]`);
  ...
  console.log(`Inserted/updated subscription [${result.id}] for user [${result.userId}]`);
...
export const upsertPaymentIntentRecord = async (paymentIntentId: string, customerId: string) => {
  // Get customer's UUID from mapping table.
  const user = await prisma.user.findUnique({
    where: { stripe_customer_id: customerId },
  });
  ...
  const payment = {
    ...
    canceled_at: paymentIntent.canceled_at ? toDateTime(paymentIntent.canceled_at).toISOString() : null,
  };
  ...
  console.log(`Inserted/updated payment intent [${paymentIntentRecord.id}] for user [${paymentIntentRecord.userId}]`);
};
