...
export const truncateEmail = (email: string | null) => {
  ...
  let truncatedLocal = local.length <= 2 ? local : local[0] + '*'.repeat(local.length - 2) + local[local.length - 1];
...
export const numberFormat = (num: number, locale = 'en-US', fractionDigits = 0) => {
