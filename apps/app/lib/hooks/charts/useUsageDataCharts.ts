...
export const useUsageDataCharts = (startDate: Date, endDate: Date) => {
  ...
  const totalCountContext = Object.values(groupedData).reduce((acc, cur) => acc + cur.context, 0);

  const totalCountGenerated = Object.values(groupedData).reduce((acc, cur) => acc + cur.generated, 0);

  const totalCountRequests = Object.values(groupedData).reduce((acc, cur) => acc + cur.requests, 0);
