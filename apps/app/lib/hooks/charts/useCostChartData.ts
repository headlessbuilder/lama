...
export const useCostChartData = (
  ...
  const groupedData = data
    ...
    .reduce(
      (acc, cur) => {
        const date = format(new Date(cur.aggregation_timestamp * 1000), 'MMMM d, yyyy h:mm a');
        ...
        const cost =
          MODEL_COST[cur.snapshot_id as Snapshot]! * (cur.n_generated_tokens_total + cur.n_context_tokens_total);
  ...
  const groupedDalleData = data
    ...
    .reduce(
      (acc, cur) => {
        const date = format(new Date(cur.timestamp * 1000), 'MMMM d, yyyy h:mm a');
        ...
        const cost = IMAGE_MODEL_COST[cur.image_size as ImageResolution] * cur.num_images;
  ...
  const chartData = [...Object.entries(groupedData), ...Object.entries(groupedDalleData)]
  ...
  const snapshots = [...Object.keys(groupedData1), ...Object.keys(groupedDalleData1)];
