...
export const useUsageData = (startDate: Date, endDate: Date, rateLimitingEnabled: Boolean = true) => {
  ...
  return useQueries({
    ...
    queries: dates.reverse().map((date) => ({
      ...
      queryFn: () => (rateLimitingEnabled ? rateLimitQuery(openai.getUsage, date) : openai.getUsage(date)),

      // stale time of 5 minutes if day is today, otherwise infinity
      staleTime: format(date, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd') ? 1000 * 60 * 5 : Infinity,
