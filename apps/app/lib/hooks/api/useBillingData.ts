...
export const useBillingData = (startDate: Date, endDate: Date) => {
  ...
  const queryKey = ['billing', format(startDate, 'yyyy-MM-dd'), format(endDate, 'yyyy-MM-dd')];
  ...
  return useQuery({
    ...
    staleTime: format(endDate, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd') ? 1000 * 60 * 5 : Infinity,
