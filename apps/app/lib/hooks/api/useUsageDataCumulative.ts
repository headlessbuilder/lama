...
export const useUsageDataCumulative = (startDate: Date, endDate: Date) => {
  ...
  const totalTokenData = [
    ...Object.entries(cumulativeUsage.contextTokens).map(([name, value]): { name: string; value: number } => ({
      name,
      value: value as number,
    })),
    ...Object.entries(cumulativeUsage.generatedTokens).map(([name, value]): { name: string; value: number } => ({
      name,
      value: value as number,
    })),
  ]
