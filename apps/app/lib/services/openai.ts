...
import { BillingSubscriptionResponse, BillingUsageResponse, OrganizationUsers, UsageResponse } from '../types';
...
export class OpenAI {
  ...
  async getUsage(date: Date | string): Promise<UsageResponse> {
    ...
    try {
      const res = await axios.get(`https://api.openai.com/v1/usage?${new URLSearchParams(query)}`, {
        headers: {
  ...
  async getBillingUsage(startDate: Date | string, endDate: Date | string): Promise<BillingUsageResponse> {
    const key = localStorage.getItem(LOCAL_STORAGE_KEY);
    if (!key) throw new Error('OpenAI key not set');
    OpenAI.setKey(key.replaceAll('"', ''));
    ...
    const res = await axios.get<BillingUsageResponse>(
      `https://api.openai.com/dashboard/billing/usage?${new URLSearchParams(query)}`,
      {
        headers: {
          Authorization: `Bearer ${OpenAI.key}`,
          'Openai-Organization': OpenAI.orgId || '',
  ...
  async isValidKey(key: string | null): Promise<boolean> {
    ...
    try {
      const response = await axios.get(`https://api.openai.com/dashboard/billing/subscription`, {
        headers: {
