...
const useLocalStorage = <T>(key: string, initialValue?: T, poll: boolean = false): [T | null, (value: T) => void] => {
  ...
  useEffect(() => {
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === key) {
        setValue(event.newValue ? JSON.parse(event.newValue) : (initialValue ?? null));
      }
    };
