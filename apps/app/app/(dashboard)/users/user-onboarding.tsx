...
import { ArrowRightCircle, Check, Copy, CreditCard, Key, Send } from 'lucide-react';
...
const UserOnboarding = ({ code, className, onRefresh = () => {}, user_id = false }: OnboardingProps) => {
  ...
  const handlePayment = async () => {
    ...
    const paymentLink =
      subscriptionPlans[process.env.NODE_ENV as 'development' | 'production' | 'test'][plan as 'free' | 'pro'][
        'monthly'
      ];
        ...
        <Grid numItems={1} className="gap-4 w-full">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Building an AI product is hard. You probably have no idea who your power users are, how many requests
                they&apos;re making, or how much they&apos;re costing you.
              </CardDescription>
              <CardDescription className="font-semibold">Lets start answering these questions today.</CardDescription>
            </CardHeader>
            <CardContent>
              <video src="https://cdn.llm.report/users-demo.mp4" autoPlay loop muted className="rounded-xl border" />
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex-row gap-4 items-center">
              ...
              <div className="flex flex-col justify-center gap-1.5">
                ...
                <CardDescription>
                  This key will be used to identify your requests so that you can view your logs in the dashboard.
                </CardDescription>
              </div>
            </CardHeader>
            {key && (
          ...
          <Card
            ...
            <CardHeader className="flex-row gap-4 items-center">
              ...
              <div className="flex flex-col justify-center gap-1.5">
                ...
                <CardDescription>
                  Choose a plan that fits your needs. We support developers, startups, and teams of all sizes.
                </CardDescription>
              </div>
            </CardHeader>
            <CardContent>
              <Select
                ...
                <SelectTrigger>
                  <SelectValue defaultValue="free" placeholder="Free - $0/month" />
                </SelectTrigger>
                <SelectContent>
            ...
            <CardFooter className="justify-between">
              ...
              {subscribed && user.stripe_customer_id && (
                <StripePortalButton customerId={user.stripe_customer_id} className="gap-2">
                  <CreditCard className="w-4 h-4" />
          ...
          <Card
            ...
            <CardHeader className="flex-row gap-4 items-center">
              ...
              <div className="flex flex-col justify-center gap-1.5">
                ...
                <CardDescription>Update your code using the examples below, or just press the button!</CardDescription>
              </div>
            </CardHeader>
            <CardContent>
              <div className="mt-2">
              ...
              <div className="mt-2 space-y-2">
                <m.div
                  ...
                  dangerouslySetInnerHTML={{
                    __html:
                      activeTab === 'curl'
                        ? code.curl.replace('$LLM_REPORT_API_KEY', key || '$LLM_REPORT_API_KEY')
                        : activeTab === 'js'
                          ? code.js.replace('${process.env.LLM_REPORT_API_KEY}', key || 'process.e')
                          : activeTab === 'nodejs'
                            ? code.nodejs.replace('${process.env.LLM_REPORT_API_KEY}', key || 'process.e')
                            : activeTab === 'python'
                              ? code.python.replace('os.getenv("OPENAI_API_KEY")', key || 'os.getenv("OPENAI_API_KEY")')
                              : '',
                  }}
                />
              </div>
